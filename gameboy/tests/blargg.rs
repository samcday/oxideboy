...
fn run_blargg_harness_test(rom: &[u8]) {
    ...
    loop {
        ...
        if cpu.hw.cart.ram[1] == 0xDE && cpu.hw.cart.ram[2] == 0xB0 && cpu.hw.cart.ram[3] == 0x61 {
            let exit_code = cpu.hw.cart.ram[0];
            if exit_code != 0x80 {
                let mut end = 0x04;
                while cpu.hw.cart.ram[end] != 0 && end < 0x1FFF {
                    end += 1;
                }
                let output = std::str::from_utf8(&cpu.hw.cart.ram[0x04..end]).unwrap();
                if exit_code != 0 {
