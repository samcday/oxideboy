...
fn main() -> Result<()> {
    ...
    let mut gb_buffer = [0; gameboy::ppu::SCREEN_SIZE];
    let mut limit = false;
    ...
    'running: loop {
        ...
        for event in event_pump.poll_iter() {
            match event {
                ...
                Event::KeyDown {keycode: Some(Keycode::Up), ..} => { cpu.hw.joypad.up = true; }
                Event::KeyDown {keycode: Some(Keycode::Down), ..} => { cpu.hw.joypad.down = true; }
                Event::KeyDown {keycode: Some(Keycode::Right), ..} => { cpu.hw.joypad.right = true; }
                Event::KeyDown {keycode: Some(Keycode::Left), ..} => { cpu.hw.joypad.left = true; }
                Event::KeyDown {keycode: Some(Keycode::A), ..} => { cpu.hw.joypad.a = true; }
                Event::KeyDown {keycode: Some(Keycode::S), ..} => { cpu.hw.joypad.b = true; }
                Event::KeyDown {keycode: Some(Keycode::Return), ..} => { cpu.hw.joypad.start = true; }
                Event::KeyDown {keycode: Some(Keycode::RShift), ..} => { cpu.hw.joypad.select = true; }
                Event::KeyUp {keycode: Some(Keycode::Up), ..} => { cpu.hw.joypad.up = false; }
                Event::KeyUp {keycode: Some(Keycode::Down), ..} => { cpu.hw.joypad.down = false; }
                Event::KeyUp {keycode: Some(Keycode::Right), ..} => { cpu.hw.joypad.right = false; }
                Event::KeyUp {keycode: Some(Keycode::Left), ..} => { cpu.hw.joypad.left = false; }
                Event::KeyUp {keycode: Some(Keycode::A), ..} => { cpu.hw.joypad.a = false; }
                Event::KeyUp {keycode: Some(Keycode::S), ..} => { cpu.hw.joypad.b = false; }
                Event::KeyUp {keycode: Some(Keycode::Return), ..} => { cpu.hw.joypad.start = false; }
                Event::KeyUp {keycode: Some(Keycode::RShift), ..} => { cpu.hw.joypad.select = false; }
        ...
        if !paused {
            cpu.hw.apu.sample_queue.clear();
            for _  in 0..17556 {
                cpu.fetch_decode_execute();
            }

            gb_buffer.copy_from_slice(&cpu.hw.ppu.framebuffer);

            let mut samples = &cpu.hw.apu.sample_queue[..];
