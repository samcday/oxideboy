// Snapshot of the insane mess that was the PPU before I undertook (literally) the 5th rewrite of it.
        ...
        match self.mode {
            Mode::Mode0(hblank_cycles) => {
                ...
                if self.cycles == hblank_cycles {
                    ...
                    if self.ly < 144 {
                        // On the first line after enabling the LCD, we don't go to Mode2, we instead
                        // jump straight to Mode3.
                        self.mode = if self.first_frame { Mode::Mode3 } else { Mode::Mode2 };
